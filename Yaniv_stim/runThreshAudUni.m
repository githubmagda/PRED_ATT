try
    %% Auditory unimodal threshlod blocks
    thisFile = ['aUT' Params.subjID];
    ...
    while ~ok
        ...
        saveas(gcf,['./data/',Params.SubjectFolder '/audUniRoughRun' datestr(now,'yymmddHHMMSS')],'fig');
    ...
    saveas(gcf,['./data/',Params.SubjectFolder '/aUniRTHistThreshold' datestr(now,'yymmddHHMMSS')],'fig');
    ...
    while ~ok
        figure;
        [Params.audUniPFParams] = plotPF(Params.audTargLevel ,CountCorrect(1,:),OutOf(1,:),.01,0);
        
        % Find stimCorr value at wanted performance
        Params.audUniThresholdValue = 10.^(PAL_CumulativeNormal(Params.audUniPFParams,Params.thresholdPoint,'Inverse'));
        disp([num2str(Params.thresholdPoint*100) '% performance at: ' num2str(Params.audUniThresholdValue) ' level']);
        
        title([Params.subjID 'Uni-modal auditory block PF: \lambda = '...
            num2str(lambda) ' \alpha = ' num2str(Params.audUniThresholdValue)]);
        ...
        if ~ok
            ...
            saveas(gcf,['./data/',Params.SubjectFolder '/aUniThreshold' datestr(now,'yymmddHHMMSS')],'fig');
        end
    end
catch
    sca;
