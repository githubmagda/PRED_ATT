function par = staircase_exp_config()
...
par.model = 'weibull';  % default; 'logit' the model for the psychometric function, choose 
...
%
% The Weibull psychometric fucntion:
...
par.x0 = 0.5;    % 30; the initial signal strength
par.x_lim = [.01,.99]; % default [-30 30];   % the limits to the signal strength

par.alpha = struct(...
    'limits',[.01, .99],...       % default: [-10 30]; range of the parameter space for alpha     %'limits',[-10 30],...       % default: [-10 30]; range of the parameter space for alpha
    'N',21,... %  61,...        %number of alpha values. If this value is set to 1, then the first element of alpha.limits would be the assumed alpha and the alpha parameter is not estimated.
    ...
    'mu',0,...                % 0 mean of the prior distribution.
    'std',20 ...              %standard deviation of the prior distribution.  
    );

par.beta = struct(...
    'limits',[1.1 10],...  % 'limits',[.1 10]     %range of the parameter space for beta       
    'N',90,...        %41,...   %number of beta values. If this value is set to 1, then the first element of beta.limits would be the assumed beta and the beta parameter is not estimated.
