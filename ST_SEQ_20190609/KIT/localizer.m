...
if p.useEyelink
    messageText = strcat('LOCALIZER_START', num2str(lr.number) );
...
for f = 1: p.series.stimPerSeries % number of times stimulus will be shown
    ...
    if p.useEyelink
        messageText = strcat('LOCALIZER',num2str(series.number),'TRIAL_START',num2str(f)); 
        Eyelink('Message', messageText);
    end
    ...
    for ff = 1 : p.scr.framesPerMovie
        ...
        if p.useEyelink
            messageText = strcat('Series_',num2str(lr.number), 'FlashQuadOFFset', num2str(thisPred) );
            Eyelink('message', messageText);
        end
        ...
        if p.useEyelink && fr.outOfBounds
            ...
            messageText = ['gazeOutOfBounds_', 'Series',num2str(lr.number), 'thisQuad', thisQuad];
            Eyelink('message',messageText);
        end
        ...
        % SEND EYETRACKER MESSAGE
        if p.useEyelink
            messageText = strcat(['SERIES',num2str(lr.number), 'TRIAL',num2str(tr.number),'FRAME',num2str(frame.number),'Cue', thisCue, 'Pred', thisPred, 'dotTime',]);
            Eyelink('message',messageText);
        end
