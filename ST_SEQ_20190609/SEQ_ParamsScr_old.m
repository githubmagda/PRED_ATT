...
[p.scr.frameRate, p.scr.nrValidSamples, p.scr.stddev]= Screen('GetFlipInterval', p.scr.window);
p.scr.Hz = 1/p.scr.frameRate; 
...
p.series.chunkRpts = 6;
p.series.chunkLength = 3;

% MOVIE SPECS
p.scr.stimDurationSecs = round2flips(p, .667); % .5 at 60 Hz .667 for 40 hz flash?
p.scr.framesPerMovie = round(p.scr.stimDurationSecs * p.scr.Hz); 
...
p.series.dotPerc = .02;% DEFAULT .03 = percent of dots per series
p.series.dotNumAv = floor( p.series.stimPerSeries * p.series.dotPerc);
...
p.series.dotProbStaircase = .5; % for 50% catch trials
...
p.scr.fixCoords0 = [xCoords .* ext; yCoords .* ext];
...
p.scr.waitText = round(p.waitText ./p.scr.frameRate) * p.scr.frameRate; %% time for instruction delay
p.scr.waitBlank = round(p.waitBlank ./ p.scr.frameRate) * p.scr.frameRate; %% time for blank screen intervals
