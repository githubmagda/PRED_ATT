...
p.grat.RadiusDeg         = 1.0;   % 3 (2*r = 6 (Hoogenboom, 2006?) )                          % p.scr.degPerPix .* round( p.scr.pixelsXY/8.2 ); % = 2; % grating radius (deg) 
...
p.grat.PosDeg            = 2; % 4 (Hoogenboom, 2006?) p.scr.degPerPix .* round( p.scr.pixelsXY/4.1 ); % =6; % center position of grating from window center (deg)
...
p.grat.phaseGrat                   = 0;
p.grat.freqGrat                    = 3.2 / p.scr.pixPerDeg;      % Landau & Fries, 2015 3.2    % see paper by Ayelet, Fries 2015 : 3.2 20; Martin Vinck - .11; Hoogenboom, 2006 3 cycles per degree)?
p.grat.contrastGrat                = 1.0;
...
p.grat.leftGrat          = p.scr.centerX -p.grat.sineTexRect(3)/2 -p.grat.PosSide;
p.grat.rightGrat         = p.scr.centerX -p.grat.sineTexRect(3)/2 +p.grat.PosSide;
p.grat.topGrat           = p.scr.centerY -p.grat.sineTexRect(4)/2 -p.grat.PosSide;
p.grat.bottomGrat        = p.scr.centerY -p.grat.sineTexRect(4)/2 +p.grat.PosSide;
p.grat.offsetXSet        = [ p.grat.leftGrat, p.grat.rightGrat, p.grat.rightGrat, p.grat.leftGrat];
p.grat.offsetYSet        = [ p.grat.topGrat, p.grat.topGrat, p.grat.bottomGrat, p.grat.bottomGrat];

% setup destination rects for gratings
% make destination rects for gratings
p.grat.dstRectGrats        = OffsetRect( 2 * p.grat.sineTexRect, p.grat.offsetXSet', p.grat.offsetYSet')';
p.grat.paramsGrats         = repmat([p.grat.phaseGrat, p.grat.freqGrat, p.grat.contrastGrat, 0], 4, 1)';
