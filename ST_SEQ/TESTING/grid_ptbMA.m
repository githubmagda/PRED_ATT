...
try
    ...
    quadRect = [0 0 round(sizeQuadX) round(sizeQuadY)];
    ...
    maskDotOuter = createCirclesMask( [ sizeMain+1, sizeMain+1 ], [ xCenter, yCenter ], outerHypot, 'outer');
    maskDotInner = createCirclesMask( [ sizeMain+1, sizeMain+1 ], [ xCenter, yCenter ], innerHypot, 'outer');
    
    mask2 = createCirclesMask([sizeQuadX+1, sizeQuadX+1],[(sizeQuadX+1)/2, (sizeQuadX+1)/2], radiusQuadCircle - 15, 'inner');
    mask2(mask2 == 1) = 2;
    
    maskMainDiff = maskDotOuter + maskDotInner; % overlay rings to give 1s for annulus
    maskMainDiff(maskMainDiff == 2) = 4;
    maskMainDiff2 = maskMainDiff( 1:length(mask2), 1:length(mask2));
    maskMainDiff3 = maskMainDiff2 + mask2;
    [rowDotSet, colDotSet] =  find(maskMainDiff3 == 2);
    ...
    for c = 1 : stim2Present
        ...
        for cc=1:numFrames % e.g. 60
            ...
            mask1 = createCirclesMask([sizeQuadX+1, sizeQuadX+1],[(sizeQuadX+1)/2, (sizeQuadX+1)/2], radiusQuadCircle, 'outer');
            m(mask1) = 0;
    ...
    if thisCue == 1
        ...
        switch randChoose
            ...
            case 2
                attn = attn2;
                rowDotSet = rowDotSet + sizeQuadX;                
            case 3
                attn = attn3;
                rowDotSet = rowDotSet + sizeQuadX;
                colDotSet = colDotSet + sizeQuadY;
            case 4
                attn = attn4;
                colDotSet = colDotSet + sizeQuadY;
        end
