function[] = test_makePredSeriesReplaceNoRptEven

elements = [1,2,3,4]; %p.series.seqBasicSet; % numbers to select from  % = 4

chunkLength = 4; % p.series.chunkLength;  % = 3
nRpt = 5; %p.series.chunkRpts; % how many times chunk will be repeated % = 5
lengthInStim = 120; %p.series.lengthInStim;
    ...
    if strcmp( type, 'order') 
        ...
        if lenTest > 3 && mod( lenTest,2) == 0 % testChunk has even number of elements;
            ...
            if strcmp( num2str( testChunk( 1 : lenTest/2)), num2str( testChunk( lenTest/2+1 : lenTest)))
                ...
                selectFrom = Shuffle( selectFrom( selectFrom ~= nextElement)); % remove problematic last element from set
        ...
        if ( lenTest - len) == 0 && ( firstEl == nextElement) % last element in ordered chunk and same first/last elements
            selectFrom = Shuffle( selectFrom( selectFrom ~= firstEl));
            nextElement = selectFrom( 1); % change nextElement
        end
